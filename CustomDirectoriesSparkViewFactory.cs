// Taken from http://www.koders.com/csharp/fid85FA257C07DBF120974250C791B01A033EF764BA.aspx?s=ajax
// Modified to support additional search locations
...
namespace Spark.Web.Mvc.Customised
{
	public class CustomDirectoriesSparkViewFactory : IViewEngine, IViewFolderContainer, ISparkServiceInitialize
	{
		private ISparkViewEngine _engine;
		private readonly List<string> _customisedViewLocations;


		public CustomDirectoriesSparkViewFactory()
			: this(null)
		{
		}

		public CustomDirectoriesSparkViewFactory(ISparkSettings settings)
		{
		...
		public virtual void AddSubDirectoryViewLocation(string directoryName)
		{
			_customisedViewLocations.Add(directoryName);
		}
		...
		{
			var newViewLocations = CreateCustomisedViewLocations(controllerName, viewName);
		...
		private IEnumerable<string> CreateCustomisedViewLocations(string controllerName, string viewName)
		{
			List<string> newViewLocations = new List<string>();

			foreach (var customisedViewLocation in _customisedViewLocations)
			{
				newViewLocations.Add(customisedViewLocation + "\\" + controllerName + "\\" + viewName + ".spark");
				newViewLocations.Add(customisedViewLocation + "\\" + viewName + ".spark");
			}
